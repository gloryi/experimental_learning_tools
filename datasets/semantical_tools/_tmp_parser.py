...
def chain_two_features(f1, f2, mnemonic_dict):
    ...
    cross_1 = lambda _: len(ft1s.difference(set(_[: len(_) // 2]))) + abs(
        len(_) - len(f1)
    )
    cross_2 = lambda _: len(ft2s.difference(set(_[len(_) // 2 :]))) + abs(
        len(_) - len(f2)
    )
...
with open(
    os.path.join(os.getcwd(), "learning_sets", "hanzi_2", "raw_features.csv"), "w"
) as hanzi_prepared:
